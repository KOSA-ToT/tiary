...
const userId = "test";
const auth = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJ0ZXN0QGdtYWlsLmNvbSIsImlkIjoxLCJleHAiOjE3MDI1MjU3NjQsImVtYWlsIjoidGVzdEBnbWFpbC5jb20ifQ.jdOz0_SOzGEJu345lNPhenY9e0Qjt8MTxEcat8KrrsLbLx-tdEHf1-ZaUY6swBOw1h4acmWDWw4g3er1ONzQUA"
...
function createComment() {
  if (commentRequestDTO.value.content.trim() == "") {
    ...
    if (userId) {
      axios
        .post(
          `http://localhost:8088/comment/1014`,
          commentRequestDTO.value,{headers:{
            Authorization:auth
          }}
        )
        .then((response) => {
