...
public class CommentController {
    ...
    public ResponseEntity<CommentResponseDTO> updateComment(
            @PathVariable("comment-id") Long commentId, @RequestBody CommentRequestDTO commentRequestDTO, @AuthenticationPrincipal UserDto users) {
        return ResponseEntity.status(HttpStatus.RESET_CONTENT)
                .body(commentService.update(commentId, commentRequestDTO, users.getUsers().getId()));
    }
    ...
    public ResponseEntity<CommentResponseDTO> guestUpdateComment(
        ...
        return ResponseEntity.status(HttpStatus.RESET_CONTENT)
                .body(commentService.guestUpdate(commentId, commentRequestDTO));
    }

    // 회원 댓글 삭제
    @DeleteMapping("/{comment-id}")
    public ResponseEntity deleteComment(@PathVariable("comment-id") Long commentId, @AuthenticationPrincipal UserDto users) {
        return ResponseEntity.status(HttpStatus.RESET_CONTENT).body(commentService.delete(commentId, users.getUsers().getId()));
    }

    // 비회원 댓글 삭제
    @DeleteMapping("/guest/{comment-id}")
